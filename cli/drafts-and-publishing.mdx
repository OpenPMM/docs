...
A Facebook feed accepts images only. If a feed request contains video,
OpenPMM returns a non-retryable `422` error before publication. Use the `reel`
placement for one video, or attach images to the feed Post.
...
wait for provider completion unless you add `--wait`. Exit code `0` without
`--wait` means that OpenPMM accepted or completed the requested transition.
It does not always mean that the provider published the Post.
...
- If `action_required_post_ids` is not empty, stop and inspect each Post's
  `available_actions`. Do not keep polling.
