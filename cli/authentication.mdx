...
For an agent-friendly flow, return safe authorization metadata immediately,
then resume after browser approval:
...
The CLI stores the sensitive pending state locally. Neither JSON response
contains the device secret or resulting API key.
