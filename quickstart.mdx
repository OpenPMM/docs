...
<Steps>
  ...
  <Step title="Publish the Post">
    ...
    curl --request POST \
      ...
      --data '{"confirmed":true,"when":"now","time_zone":"UTC","posts":[{"id":"post_01JABCDEF","version":1,"destination_id":"'"$OPENPMM_DESTINATION_ID"'"}]}'
    ```
  </Step>
  <Step title="Inspect the Post">
    ...
    ```bash
    curl --request GET \
      --url "https://api.openpmm.com/v1/workspaces/$OPENPMM_WORKSPACE_ID/posts/post_01JABCDEF?include=attempts" \
      --header "Authorization: Bearer $OPENPMM_API_KEY"
    ```
