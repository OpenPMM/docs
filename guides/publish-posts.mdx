...
<Steps>
  ...
  <Step title="Publish the same Post">
    ...
    OpenPMM returns `202 Accepted` while immediate publication continues.
    The response contains every Post ID. OpenPMM returns status `422` when a
    file needs your decision.
  </Step>
  <Step title="Check the Post status">
    ...
    Poll while `terminal` is `false` and no action is required. Use
    `retry_safety` before you retry a failed Post.
  </Step>
</Steps>
