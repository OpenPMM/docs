...
Use the same idempotency key when you retry the same request. Use a new key
when the message or Workspace changes.
...
you must retry a request with the same key.
