...
OpenPMM checks the file type and the file size when you attach the image. It converts a GIF, and makes an image smaller when it is above 2,000,000 bytes. See [Convert media](/guides/convert-media). Bluesky shows a still image, so OpenPMM does not convert an animated image.
